...
const xml_schedule_2_day = async (req, res, next) => {
    try {
        const currentDate = new Date();
        const tomorrow = new Date(currentDate);
        tomorrow.setDate(currentDate.getDate() + 1); // Lấy ngày mai
        ...
        const formattedDate = formatDate(tomorrow); // Sử dụng ngày mai
        const results = await crawlScheduleApi(formattedDate);
        ...
        console.error("Error crawl schedule for tomorrow : ", error);
    }
};

const xml_schedule_3_day = async (req, res, next) => {
    try {
        const currentDate = new Date();
        const tomorrow = new Date(currentDate);
